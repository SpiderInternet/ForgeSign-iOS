...
struct CertificatesSheet: View {
    ...
    @EnvironmentObject private var storeObj: CertificateStore
    ...
    var body: some View {
        NavigationStack {
            List {
                ...
                CertificatesListSection(store: storeObj)
            }
            ...
            .fileImporter(
                ...
                if case .success(let urls) = result, let url = urls.first {
                    storeObj.importProvisioningProfile(from: url)
                }
            }
            .alert("كلمة سر الشهادة", isPresented: $showPasswordAlert) {
                ...
                Button("استيراد") {
                    if let url = selectedP12URL {
                        storeObj.importP12(from: url, password: passwordInput)
                        passwordInput = ""
                    }
                }
