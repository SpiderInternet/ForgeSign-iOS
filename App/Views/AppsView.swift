    ...
    var filteredApps: [FeedApp] {
        if searchText.isEmpty {
            ...
            return store.apps.filter { app in
                let nameMatch = app.title?.localizedCaseInsensitiveContains(searchText) ?? false
                let descMatch = app.description?.localizedCaseInsensitiveContains(searchText) ?? false
